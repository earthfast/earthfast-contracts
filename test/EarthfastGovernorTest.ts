...
describe("EarthfastGovernor", function () {
  ...
  let governor: EarthfastGovernor;
  let timelock: EarthfastTimelock;
  ...
  it("Should allow zero admin", async function () {
    const factory = await hre.ethers.getContractFactory("EarthfastGovernor");
    const governor = <EarthfastGovernor>await factory.deploy(ZeroAddress, tokenAddress, timelockAddress, 0, 25, 0, 51);
