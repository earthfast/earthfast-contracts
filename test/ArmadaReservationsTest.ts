...
describe("ArmadaReservations", function () {
  ...
  it("Should disallow impl calls from unauthorized senders", async function () {
    ...
    await expect(reservations.deleteReservationImpl(HashZero, HashZero, { last: false, next: false })).to.be.revertedWith("not impl");
    ...
    await expect(reservations.connect(newRegistry.signer).deleteReservationImpl(HashZero, HashZero, { last: false, next: false })).to.be.revertedWith("not impl");
  });
});
