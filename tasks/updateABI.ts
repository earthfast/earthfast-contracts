...
task("updateABI", "Updates deployment files with new ABIs after contract upgrades")
  ...
  .addOptionalParam(
    "implementations",
    "Comma-separated list of implementation names (if different from contract names)"
  )
  ...
  .setAction(async (args, hre) => {
    ...
    if (implementations.length !== contractNames.length) {
      console.warn(
        `Warning: Number of implementations (${implementations.length}) doesn't match number of contracts (${contractNames.length}). Using contract names as implementation names where needed.`
      );
