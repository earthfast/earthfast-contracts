...
contract EarthfastOperators is AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
  ...
  mapping(bytes32 => EarthfastOperator) private _operators;
  ...
  function initialize(address[] calldata admins, EarthfastRegistry registry, uint256 stakePerNode, bool grantImporterRole)
  public initializer {
    __Context_init();
    __ERC165_init();
  ...
  function unsafeImportData(EarthfastOperator[] calldata operators, bool revokeImporterRole)
  ...
  public virtual onlyAdmin {
    ...
    for (uint256 i = 0; i < n; i++) {
      EarthfastOperator storage operator = _operators[_operatorIds.at(skip + i)];
  ...
  public onlyAdmin whenNotReconciling whenNotPaused returns (bytes32 operatorId) {
    ...
    _operators[operatorId] = EarthfastOperator({id: operatorId, owner: owner, name: name, email: email, stake: 0, balance: 0});
  ...
  public virtual view returns (EarthfastOperator memory) {
    EarthfastOperator storage operator = _operators[operatorId];
    require(operator.id != 0, "unknown operator");
    return operator;
  }
  ...
  public virtual view returns (EarthfastOperator[] memory values) {
    uint256 length = _operatorIds.length();
    uint256 n = Math.min(size, length > skip ? length - skip : 0);
    values = new EarthfastOperator[](n);
    for (uint256 i = 0; i < n; i++) {
